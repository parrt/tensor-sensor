...
def pyviz(statement, frame,
          matrixcolor="#cfe2d4", vectorcolor="#fefecd",
          gtype="digraph", gname="G") -> graphviz.Source:
...
def astviz(statement):
    ...
    for nd in ops:
        ...
        for sub in nd.kids:
            if tsensor.analysis._shape(sub.value) is None:
                continue
        label = internal_label(nd)
        gr += f'node{id(nd)} [shape=box penwidth=0 margin=0 height=.03 label=<{label}>]\n'
        # gr += f'node{id(nd)} [shape=box penwidth=0 height=.3 margin=0 label=<<font face="Consolas" color="#444443" point-size="12">{nd}</font>>]\n'
    ...
    for nd in nodes:
        ...
        for sub in kids:
            if sub in atomsS:
                gr += f'node{id(nd)} -> leaf{id(sub.token)} [dir=back, penwidth="0.5", color="#444443", arrowsize=.3];\n'
            else:
                gr += f'node{id(nd)} -> node{id(sub)} [dir=back, penwidth="0.5", color="#444443", arrowsize=.3];\n'
